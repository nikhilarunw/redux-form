...
describe('write', function () {
  ...
  it('should put simple arrays', function () {
    var data1 = write('cat', ['foo', 'bar'], {});
    expect(data1.cat).toBeA('array').toEqual(['foo', 'bar']);
    var data2 = write('dog', ['rabbit', 42], {});
    expect(data2.dog).toBeA('array').toEqual(['rabbit', 42]);
  });

  it('should put simple indexless array values', function () {
    var data = write('cat[]', 'meow', {
      cat: [1, 2, 3, 4]
    });
    expect(data.cat).toBeA('array').toEqual(['meow', 'meow', 'meow', 'meow']);
  });

  it('should put arrays as indexless array values', function () {
    var data = write('cat[]', ['meow', 'woof'], {
      cat: [1, 2, 3, 4]
    });
    expect(data.cat).toBeA('array').toEqual(['meow', 'woof']);
  });
  ...
  it('should put simple array values', function () {
    var data1 = write('cat[0]', 'foo', {});
    expect(data1.cat).toBeA('array');
    expect(data1.cat[0]).toBe('foo');
    var data2 = write('dog[2]', 'rabbit', {});
    expect(data2.dog).toBeA('array');
  ...
  it('should put complex array values', function () {
    var data = write('cat[0].dog[7].rabbit.fox', 'wolf', {});
  ...
  it('should only modify values along path', function () {
    ...
    var result1 = write('a.b', 'dog', initial);
    ...
    var result2 = write('c.e.g.h', 'dog', initial);
    expect(backup).toEqual(initial); // initial not mutated
    ...
    var result3 = write('j[1].l', 'dog', initial);
    expect(backup).toEqual(initial); // initial not mutated
  ...
  it('should use mutator if value is function', function () {
    var result1 = write('foo', function (value) {
      return value + 'bar';
    }, { foo: 'candy' });
    expect(result1.foo).toBe('candybar');

    var result2 = write('foo.bar.dog', function (value) {
      return value * 2;
    }, {
      foo: {
        bar: {
    ...
    var result3 = write('foo.bar', function (value) {
    ...
    var result4 = write('foo', function (value) {
